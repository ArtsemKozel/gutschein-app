...
async function searchVoucher() {
    ...
    if (!voucher) {
        document.getElementById('voucher-result').innerHTML = `<div class="error-message">❌ Gutschein nicht gefunden!</div>`;
        return;
    }
    
    if (voucher.status === 'cancelled') {
        document.getElementById('voucher-result').innerHTML = `<div class="error-message">🚫 Dieser Gutschein wurde storniert</div>`;
        return;
    }
    
    if (voucher.status === 'expired') {
        document.getElementById('voucher-result').innerHTML = `<div class="error-message">⏰ Dieser Gutschein ist abgelaufen</div>`;
        return;
    }
    
    if (voucher.status === 'redeemed') {
        document.getElementById('voucher-result').innerHTML = `<div class="error-message">✅ Dieser Gutschein wurde bereits vollständig eingelöst</div>`;
        return;
    }
...
async function toggleVoucherCard(cardElement, voucherId) {
    ...
    if (transactions.length > 0) {
        historyHTML = `<div class="transaction-history"><strong>Einlösungs-Historie:</strong><ul>`;
    ...
    detailsDiv.innerHTML = `
        <div class="detail-item"><span class="label">Wert:</span><span class="value">${parseFloat(voucher.original_value).toFixed(2)} €</span></div>
        <div class="detail-item"><span class="label">Eingelöst:</span><span class="value">${redeemedAmount.toFixed(2)} €</span></div>
        <div class="detail-item"><span class="label">Verfügbar:</span><span class="value">${parseFloat(voucher.remaining_value).toFixed(2)} €</span></div>
        <div class="detail-item"><span class="label">Gültig bis:</span><span class="value">${expiryDate}</span></div>
        ${voucher.buyer_name ? `<div class="detail-item"><span class="label">Käufer:</span><span class="value">${voucher.buyer_name}</span></div>` : ''}
        ${historyHTML}
        ${isAdmin() && voucher.status === 'active' ? `<button onclick="confirmCancelVoucher('${voucher.id}', '${voucher.code}')" style="background-color: #8B5A3C; margin-top: 15px;">🚫 Gutschein stornieren</button>` : ''}
    `;
}
